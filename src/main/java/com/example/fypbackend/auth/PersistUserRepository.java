...
public interface PersistUserRepository extends CrudRepository<PersistUser, Integer> {
    ...
    @Query(value = "SELECT * FROM `users` INNER JOIN `users_groups` ON users_groups.user_id = users.id where role = \"PATIENT\" and users_groups.groups_id in ( SELECT groups_id FROM `users_groups` INNER JOIN `groups` ON groups.id = users_groups.groups_id WHERE user_id = 1 AND is_mdt = true) ",
            nativeQuery = true)
